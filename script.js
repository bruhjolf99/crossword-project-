...
// Returns the current language strings object (EN or BM) based on appSettings.language
function t() {
  return STRINGS[appSettings.language] || STRINGS.en;
}

// Loads app settings from localStorage, falls back to DEFAULT_APP_SETTINGS if none saved
...
// Applies current appSettings to the DOM — cell size CSS vars, contrast, animations, etc.
...
// Updates all translatable text in the DOM based on current language setting (EN/BM)
function applyLanguage() {
  ...
  const map = {
    ...
    // Landing page
    'landingLabel':          'Pendidikan Moral Tahun 5',
    'landingSubtitle':       isEN ? 'Welcome' : 'Selamat Datang',
    'landingTriviaTag':      '📖📖📖📖📖📖📖',
    ...
    'tetLblDarkMode':        isEN ? 'Dark Mode'                    : 'Mod Gelap',
...
// Loads game settings (timer, scoring, version, hints) from localStorage
function loadSettings() {
  const saved = localStorage.getItem('gameSettings');
  return saved ? { ...DEFAULT_SETTINGS, ...JSON.parse(saved), gameVersion: 1, hintsMode: 'fixed' } : { ...DEFAULT_SETTINGS };
}

// Saves provided game settings object to localStorage
function saveSettings(settings) {
  localStorage.setItem('gameSettings', JSON.stringify(settings));
}

// Returns a fresh copy of DEFAULT_GAME_SETTINGS
...
// Returns max reroll count based on puzzle size
function getMaxRerolls() {
  return gameSettings.gameVersion === 2 ? 3 : 5;
}

// Initialises the activeHints map and reroll counter for a new game session
function initHints() {
...
// Generates hints for a specific puzzle based on current hintsMode (fixed/random/off)
...
// Picks a random subset of cells to reveal as hints for the given puzzle
...
// Applies stored hints for a puzzle to the DOM — fills cells and marks them as hint cells
...
// Removes hint styling from all cells in a puzzle, then fires optional callback
...
// Regenerates random hints for the current puzzle if rerolls remain, costs 1 reroll
...
// Re-applies bold styling to all hint cells in a puzzle (used after DOM rebuilds)
...
// Adds hint-cell class to the wrapper of a given input cell
...
// Removes hint-cell class from a cell wrapper
...
// Enables or disables the reroll button based on remaining rerolls and hints mode
...
// Saves the full current game session to localStorage (puzzle states, lives, timer, settings)
...
// Loads a saved session from localStorage and restores all game state
...
// Deletes the saved session from localStorage
function clearSession() {
  localStorage.removeItem(SESSION_KEY);
}

// Returns true if a valid saved session exists in localStorage
function hasSavedSession() {
  return !!localStorage.getItem(SESSION_KEY);
}

// Shows or hides the Continue button based on whether a saved session exists
...
// Loads scoreboard entries array from localStorage
...
// Saves a new entry to the scoreboard, keeping max 10 entries
function saveScoreboardEntry(entry) {
  let board = loadScoreboard();
  board.push(entry);
  // Sort: score DESC, then lives DESC, then date DESC
  board.sort((a, b) => {
...
// Wipes all scoreboard entries from localStorage
function clearScoreboard() {
  localStorage.removeItem(SCOREBOARD_KEY);
}

// Builds and saves a new scoreboard entry from current game results
function addScoreboardEntry(totalScore, livesLeft, version, outcome) {
  ...
  // Detect if non-default settings were used for this version
  const vDefaults = version === 1
    ? { timerEnabled: true, timeLimit: 60, scoringEnabled: true, gameVersion: 1, hintsMode: 'fixed' }
    : { timerEnabled: true, timeLimit: 60, scoringEnabled: true, gameVersion: 2, hintsMode: 'random' };
  const isCustom = Object.keys(vDefaults).some(k => gameSettings[k] !== vDefaults[k]);
  saveScoreboardEntry({
...
// Renders all scoreboard entries into the scoreboard modal table
function renderScoreboard() {
  ...
  function buildTable(entries, label) {
    if (entries.length === 0) {
      return `<div class="sb-version-block">
        <h3 class="sb-version-title">${label}</h3>
        <p class="scoreboard-empty">${t().noRecords}</p>
      </div>`;
    }
    const rows = entries.map((entry, i) => {
      const rankDisplay = i < 3
        ? `<span class="sb-rank sb-rank--${['gold','silver','bronze'][i]}">${medals[i]}</span>`
        : `<span class="sb-rank sb-rank--plain">${i + 1}</span>`;
      const modeLabel = entry.custom
        ? `<span class="sb-mode sb-mode--custom">Customized</span>`
        : `<span class="sb-mode sb-mode--default">Default</span>`;
      const outcomeDisplay = entry.outcome !== null
        ? (entry.outcome ? t().win : t().lose)
        : '—';
      const livesMax = entry.maxLives || (entry.version === 2 ? 5 : 8);
      return `
        <tr class="${i < 3 ? 'sb-row--top' : ''}">
          <td>${rankDisplay}</td>
          <td>${entry.date} ${entry.timezone || ''}</td>
          <td>${modeLabel}</td>
          <td><strong>${entry.score}</strong> / ${entry.maxScore}</td>
          <td>❤️ ${entry.livesLeft} / ${livesMax}</td>
    ...
    return `<div class="sb-version-block">
      ...
      <table class="scoreboard-table">
        <thead><tr>
          <th>#</th><th>${t().sbDate || 'Date'}</th><th>Mode</th>
          <th>${t().sbScore || 'Score'}</th><th>${t().sbLives || 'Lives'}</th>
          <th>${t().sbResult || 'Result'}</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`;
  ...
  const v1 = board.filter(e => e.version === 1 || e.version === '1' || (!e.version && true));
  const v2 = board.filter(e => e.version === 2 || e.version === '2');

  content.innerHTML = buildTable(v1, 'Version 1') + buildTable(v2, 'Version 2');
}
...
// Updates the timer and lives summary text shown in the launcher header
...
// Wires hover effects on reset button — live tooltip with count, clears on leave
...
// Plays shake animation on lives display and reset button when a life is lost
...
// Returns max lives for current game version (V1=8, V2=5)
function getMaxLives() {
  return gameSettings.gameVersion === 2 ? 5 : 8;
}

// Resets lives to max and re-renders the lives display
function initLives() {
  livesRemaining = getMaxLives();
  renderLives();
}

// Deducts one life, triggers shake animation, disables reset if no lives left
function spendLife() {
...
// Renders the lives display as heart icons based on livesRemaining
...
// Enables or disables the reset button based on whether lives remain
...
// Returns points per correct word for a given puzzle (100 / word count)
...
// Checks if all cells of a word are filled with correct letters in the DOM
...
// Calculates and returns total score for a puzzle based on correct words in DOM
...
// Returns total word count for a given puzzle
...
// Returns the grid dimensions {rows, cols} for a puzzle
...
// Saves current DOM input values and semak status for a puzzle to puzzleStates
...
// Restores saved puzzle state back into the DOM for a given puzzle
...
// Marks a puzzle as having been checked (semak) at least once
...
// Returns how many puzzles have been checked at least once
function getSemakCount() {
  return Object.values(puzzleStates).filter(s => s && s.semakDone).length;
}

// Updates the status badge on a puzzle card (locked/in progress/complete)
...
// Refreshes status badges on all puzzle cards in the launcher
...
// Enables or disables the Submit All button based on completion and lives remaining
...
// Handles Submit All — validates all puzzles checked, calculates final score, shows result
let isSubmitting = false; // True while final score modal is open — blocks spam
let hasSubmitted = false; // True after submit — blocks re-save until new game starts

function handleFinalize() {
  if (isSubmitting) return; // Prevent spam and re-submission
...
// Builds and displays the final score modal with per-puzzle breakdown table
function showFinalScore() {
  isSubmitting = true;  // Lock while modal is open
  hasSubmitted = true;   // Block re-save until new game
  ...
  // Save to scoreboard and immediately clear session — game is over
  ...
  clearSession(); // Remove save file — submitted games can't be continued
...
// Resets the internal grid array to blank based on puzzle dimensions
function initializeGrid() {
  return Array.from({ length: CONFIG.rows }, () => Array(CONFIG.cols).fill(null));
}

// Places a word into the internal grid array at given position and direction
...
// Places all words for the current puzzle into the grid array
function buildGrid() {
...
// Assigns across/down clue numbers to grid cells based on word start positions
...
// Builds the crossword HTML table from the grid array and injects it into the DOM
...
// Attaches keydown/input handlers to a single crossword cell input element
...
// Wires global keyboard shortcuts (arrow keys, backspace, tab) for crossword navigation
...
// Moves focus to the next valid cell in a given direction from current position
...
// Highlights the active cell's corresponding clue in the clues panel
...
// Checks all filled cells against correct answers, colours cells green/red/yellow, costs a life
...
// Shows result info modal after checking answers — correct, wrong, or incomplete
...
// Clears all user input in the current puzzle and re-applies hints
...
// Wires the Check Answers button to checkAnswers()
...
// Wires the Reset button to resetPuzzle(), replacing any previous listener
...
// Sets up puzzle metadata (clue numbers, directions) for the currently loaded puzzle
...
// Rebuilds the Across and Down clue lists in the clues panel for the current puzzle
...
// Loads a puzzle by key — saves current state if needed, builds grid, restores state if saved
...
// Starts the countdown timer from a given number of minutes
...
// Resumes the timer from the current remainingSeconds value
...
// Internal timer tick — decrements remainingSeconds and updates display each second
function runTimer() {
  const display = document.getElementById('timerDisplay');
  if (!display) return;
  // Updates the timer display with current MM:SS and applies warning/danger colours
...
// Stops the timer interval without resetting remainingSeconds
function stopGameTimer() {
  if (timerInterval) { clearInterval(timerInterval); timerInterval = null; }
}

// Called when timer hits zero — auto-checks answers and shows time-up modal
...
// Shows the generic info modal with a title, message, and single OK button
...
// Shows the confirm modal with title, message, and confirm/cancel buttons
...
// Syncs all settings screen UI elements to current gameSettings/appSettings
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // --- MAIN MENU ---
  // Starts a new game session — optionally goes to modifier settings or straight to launcher
function startNewGame(goToSettings) {
  hasSubmitted = false; // Fresh game
  isSubmitting = false;
    if (hasSavedSession()) {
      showConfirmModal(
        t().newGameTitle,
  ...
  // Opens the modifier settings screen and takes a snapshot for dirty-checking
function openModifierSettings() {
    modifierSnapshot = JSON.parse(JSON.stringify(gameSettings));
    updateModifierDirty();
  }
  // Returns true if game settings have unsaved changes since modifier screen opened
function isModifierDirty() {
  ...
  // Returns true if current game settings match DEFAULT_GAME_SETTINGS
function isModifierAtDefault() {
    return JSON.stringify(gameSettings) === JSON.stringify(DEFAULT_SETTINGS);
  }
  // Updates modifier dirty state and non-default banner visibility
function updateModifierDirty() {
  ...
  // Takes a deep copy of appSettings as baseline for Tetapan dirty-checking
function snapshotSettings() {
    return JSON.parse(JSON.stringify(appSettings));
  }

  // Returns true if Tetapan draft settings differ from the saved snapshot
function isDirty() {
    if (!tetapanSnapshot || !tetapanDraft) return false;
    return JSON.stringify(tetapanDraft) !== JSON.stringify(tetapanSnapshot);
  }

  // Opens the Tetapan settings screen and initialises draft and snapshot
function openTetapan() {
  ...
  // Syncs all Tetapan toggle and button states to current tetapanDraft
function updateTetapanUI() {
    ...
    const dmEl = document.getElementById('darkMode');
    if (dmEl) dmEl.checked = !!tetapanDraft.darkMode;
  ...
  // Enables/disables Tetapan Save and Restore buttons based on dirty and default state
function updateTetapanButtons() {
  ...
  // Commits tetapanDraft to appSettings, saves to localStorage, applies to DOM
function applyAndSaveDraft() {
  ...
  // Closes Tetapan screen and returns to main menu, refreshing language and continue UI
function closeTetapanToMenu() {
  ...
  tetapanToggle('darkMode', 'darkMode');
  ...
  // Cancel / back buttons
  // Handles Tetapan back/cancel — shows cancel modal if dirty, otherwise reverts and closes
function tetapanCancel() {
  ...
  // Handles modifier settings back/cancel — shows cancel modal if dirty, otherwise closes
function modifierCancel() {
  ...
  // Clears any running tutorial animation timer
function tutClearAnim() {
    if (tutAnimTimer) { clearTimeout(tutAnimTimer); tutAnimTimer = null; }
  }

  // Focuses a tutorial grid cell — adds highlight and hint cursor if needed
function tutFocus(el) {
  ...
  // Types a letter into a tutorial cell
function tutType(el, letter) {
  ...
  // Clears a tutorial cell content and filled styling
function tutClear(el) {
  ...
  // Returns base delay with small random variation for natural tutorial animations
function hd(base) { return base + Math.floor(Math.random() * 60) - 30; }

  // Runs tutorial page 1 animation — typing, backspace, arrow key navigation demo
function runAnim1() {
    tutClearAnim();
  ...
  // Runs tutorial page 2 animation — hint cell interaction demo
function runAnim2() {
    tutClearAnim();
  ...
  // Runs tutorial page 3 animation — mouse click, check answers, cell colour feedback demo
function runAnim3() {
    tutClearAnim();
  ...
  // Navigates tutorial to a given page, starts page animation if applicable
function tutorialGoToPage(n) {
  ...
  document.getElementById('homeSaveBtn').addEventListener('click', function () {
    ...
    // Don't save if already submitted this session
    if (!hasSubmitted && currentPuzzleKey) savePuzzleState(currentPuzzleKey);
    if (!hasSubmitted) saveSession();
  ...
  // --- IN-GAME / LAUNCHER SETTINGS MODAL ---
  // Quick-access display settings panel reachable from both the Launcher and
  // the Crossword Game Screen via the gear (⚙) icon button.
  // Changes apply as a live preview immediately; they are committed to
  // localStorage only when the modal is closed (closeInGameSettingsBtn).
  // Dark mode is NOT exposed here — use the full Settings screen (Tetapan)
  // for dark mode and language changes.
  ...
  // Opens in-game settings from the game screen header gear button
  ...
  // Syncs the in-game settings modal UI to current appSettings values
function openInGameSettings() {
  ...
  // Wires an in-game settings toggle to live-preview the setting without saving
function igToggle(id, key) {
  ...
  if (closeFinalBtn) closeFinalBtn.addEventListener('click', function () { finalScoreModal.style.display = 'none'; isSubmitting = false; });
  ...
  const xMap = {
    ...
    'closeFinalXBtn':          () => { finalScoreModal.style.display = 'none'; isSubmitting = false; },
